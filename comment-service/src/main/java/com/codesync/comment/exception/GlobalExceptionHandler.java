...
public class GlobalExceptionHandler {
    ...
    public ProblemDetail handleInvalidOp(InvalidCommentOperationException ex) {
        log.warn("Invalid operation: {}", ex.getMessage());
        ProblemDetail pd = ProblemDetail.forStatusAndDetail(HttpStatus.BAD_REQUEST, ex.getMessage());
        pd.setTitle("Invalid Comment Operation");
    ...
    public ProblemDetail handleDuplicateReaction(DuplicateReactionException ex) {
        log.warn("Duplicate reaction: {}", ex.getMessage());
        ProblemDetail pd = ProblemDetail.forStatusAndDetail(HttpStatus.CONFLICT, ex.getMessage());
        pd.setTitle("Duplicate Reaction");
        pd.setType(URI.create("https://codesync.dev/errors/duplicate-reaction"));
        pd.setProperty("timestamp", Instant.now());
        return pd;
    }
