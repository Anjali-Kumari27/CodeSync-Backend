...
/**
 * Global Exception Handler for the Execution Service.
...
public class GlobalExceptionHandler {
    ...
    public ProblemDetail handleTimeout(ExecutionTimeoutException ex) {
        log.warn("Execution timeout: {}", ex.getMessage());
        ProblemDetail pd = ProblemDetail.forStatusAndDetail(HttpStatus.REQUEST_TIMEOUT, ex.getMessage());
        pd.setTitle("Execution Timeout");
        pd.setType(URI.create("https://codesync.dev/errors/execution-timeout"));
        pd.setProperty("timestamp", Instant.now());
        return pd;
    }

    @ExceptionHandler(ExecutionException.class)
    public ProblemDetail handleExecutionError(ExecutionException ex) {
        log.error("Execution error: {}", ex.getMessage(), ex);
        ProblemDetail pd = ProblemDetail.forStatusAndDetail(HttpStatus.SERVICE_UNAVAILABLE, ex.getMessage());
        pd.setTitle("Execution Error");
        pd.setType(URI.create("https://codesync.dev/errors/execution-error"));
        pd.setProperty("timestamp", Instant.now());
        return pd;
    }
