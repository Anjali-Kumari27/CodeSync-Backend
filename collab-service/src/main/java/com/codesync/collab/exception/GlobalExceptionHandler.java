...
/**
 * Global Exception Handler for the Collab Service.
 * RFC 7807 ProblemDetail for HTTP endpoints.
 * WebSocket errors are handled separately in the STOMP controller.
 */
...
public class GlobalExceptionHandler {
    ...
    public ProblemDetail handleSessionNotActive(SessionNotActiveException ex) {
        log.warn("Session not active: {}", ex.getMessage());
        ProblemDetail pd = ProblemDetail.forStatusAndDetail(HttpStatus.BAD_REQUEST, ex.getMessage());
        pd.setTitle("Session Not Active");
        pd.setType(URI.create("https://codesync.dev/errors/session-not-active"));
        pd.setProperty("timestamp", Instant.now());
        return pd;
    }
